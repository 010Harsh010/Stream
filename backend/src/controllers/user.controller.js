...
const sendmessage = asynHandler(async (req, res) => {
  try {
    ...
    if (!accessToken) {
      throw new ApiError(
        "Access token refresh failed. Please re-authenticate Gmail."
      );
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        refreshToken: user.GrantToken, // use stored refresh token
        accessToken: accessToken, // new token
      },
    });
...
const registerUser = asynHandler(async (req, res) => {
  ...
  //     }
  //   );
  //   //  console.log("hostaccessTokenResponse",hostaccessTokenResponse);

  //   const hostnewAccessTokenData = await hostaccessTokenResponse.json();
  //   //  console.log("newAccesstokenData",newAccessTokenData);

  //   const hostaccessToken = await hostnewAccessTokenData.access_token;
